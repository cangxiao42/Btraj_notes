...
{   
    ...
    // 这里论文中采用fast match算法，默认也是用fm方法，不过用什么方法不重要，只要搜出路径即可
    if(_is_use_fm)
    {
        ros::Time time_1 = ros::Time::now();
        ...
        corridor = corridorGeneration(path_coord, time); // 在此处构建安全走廊
...
int main(int argc, char** argv)
{
    ...
    _odom_sub = nh.subscribe( "odometry",  1, rcvOdometryCallbck); // 订阅里程计，更新无人机位置
    _pts_sub  = nh.subscribe( "waypoints", 1, rcvWaypointsCallback ); // 订阅目标点，在回调函数中调用规划函数
